...
function mostraDomande() {
  ...
  let risposte = [domandaCorrente.correct_answer, ...domandaCorrente.incorrect_answers] // i tre puntini unisce due ogetti dentro l'arrray
shuffleArray(risposte)
  ...
  for (const risposta of risposte) {
    ...
    li.appendChild(testoRisposta) // appendChild allega 
  ...
  function shuffleArray(array) {
    for (let i = array.length -1;  i > 0; i--) {
      let x = Math.floor(Math.random() * (i + 1)); // ordina in modo randomico gli elementi
  ...
  function mostraRisultato() {
    ...
    let paragrafoTesto = "Hai risposto correttamente a "+ conteggioRisposte + " domande su " + questions.length;
    testoRisultato.innerText = paragrafoTesto;
...
window.onload = mostraDomande // mostra al caricamento della pagina 
